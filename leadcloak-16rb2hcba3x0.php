<?php
/**
 * Campaign: StephanieNRhodes242
 * Created: 2022-02-21 15:24:39 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rb2hcba3x0';

$campaignSignature = 'aYPBKw20xcX16lH8XpOMGQpSrElw1vxsXgy7yHwx35VgU0vSw4';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rb2hcba3x0', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rb2hcba3x0'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rb2hcba3x0');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: aYPBKw20xcX16lH8XpOMGQpSrElw1vxsXgy7yHwx35VgU0vSw4';
...
{
    return array_key_exists('16rb2hcba3x0', $_COOKIE) ? $_COOKIE['16rb2hcba3x0'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rb2hcba3x0', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rb2hcba3x0.log';
